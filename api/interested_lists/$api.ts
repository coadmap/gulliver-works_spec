...
import type { Methods as Methods1 } from './_interested_list_id@string/recruitments'
import type { Methods as Methods2 } from './_interested_list_id@string/recruitments/_id@string'

const api = <T>({ baseURL, fetch }: AspidaClient<T>) => {
  const prefix = (baseURL === undefined ? 'http://localhost:3000/v1' : baseURL).replace(/\/$/, '')
  const PATH0 = '/interested_lists'
  ...
  return {
    ...
    _interested_list_id: (val0: string) => {
      const prefix0 = `${PATH0}/${val0}`
